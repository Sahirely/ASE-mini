...
var SocketIO = function (config) {
  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('login', function (data) {
      if (data.user != '') {
        ...
        app_clients[data.user] = data.user
        socket.to(socket.id).emit('hello', { mensaje: 'Conectado correctamente: ' + (new Date().toString()) + ' ID: ' + socket.id })
  ...
  setInterval(function () {
    for (key in app_clients) {
      ...
      //   {
      //     // method: 'GET',
      //     // uri: 'http://localhost:5100/api/push/not/' + app_clients[key].idusuario
      //   }
